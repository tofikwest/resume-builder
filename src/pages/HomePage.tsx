import { Link } from 'react-router-dom'
import { resume_img_base64 } from '../assets/icons/resume-icon.base64'
import { routes_list } from '../routes/routesList'

const HomePage = () => {
  return (
    <section className="relative mt-9 h-4/6">
      <div className=" ml-10 mt-16 flex justify-between ">
        <h1 className=" w-28 text-center text-7xl font-bold  tracking-wide">
          Star<span className="text-additional-color">t</span> buildin
          <span className="text-additional-color">g</span> you
          <span className="text-additional-color">r</span> resum
          <span className="text-additional-color">e</span>
        </h1>
        <img
          src={resume_img_base64}
          alt="random resume image"
          className="mr-32 w-96 shadow-2xl"
        />
      </div>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
        strokeWidth="1.5"
        stroke="currentColor"
        className="absolute bottom-[7px] left-[405px] h-56 w-56 rotate-180 scale-x-[-1]"
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M9 9l6-6m0 0l6 6m-6-6v12a6 6 0 01-12 0v-3"
        />
      </svg>
      <Link
        to={routes_list[1].path}
        className="absolute bottom-[-116px] left-1/3 transform rounded-2xl bg-additional-color px-16 py-5 font-bold text-white shadow-xl transition-transform hover:scale-105 hover:bg-additional-hover-color"
      >
        Build Resume
      </Link>
    </section>
  )
}

export default HomePage
